...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.nodes1 = np.transpose(self.nodes)
        # Модельная скалярная величина распределена как-то вот так
        self.smth = np.power(self.nodes[0, :], 2) + np.power(self.nodes[1, :], 2)
        self.omega = np.array([0.0, 1.0, 0.0])
        # Тут может быть скорость, но сейчас здесь нули

        self.velocity1 = np.array([np.cross(i, self.omega) for i in list(self.nodes1)])
        self.velocity = np.transpose(self.velocity1)
    ...
    def move(self, tau):
        ...
        self.omega += tau*np.cross(self.omega, A)
        print(sum(self.omega*self.omega))
        self.velocity1 = np.array([np.cross(i, self.omega) for i in list(self.nodes1)])
        self.velocity = np.transpose(self.velocity1)
...
try:
    ...
    gmsh.merge(os.path.join(path, 'hoig1.stl'))
except:
